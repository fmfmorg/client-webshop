    ...
    return (
        <div class="hidden xs:block md:hidden">
            ...
            <div ref={containerRef} class="fixed right-full top-0 w-full h-full max-w-80 bg-white z-40 peer-checked:translate-x-full transition-all duration-300">
                ...
                <div ref={buttonRef} class="grid grid-cols-2 gap-2 p-2 absolute bottom-0 left-0 w-full text-xs uppercase tracking-widest">
                    <button onClick={showResultOnClick} class="font-serif font-semibold text-center border-gray-600 border py-2 bg-gray-600 text-white mouse:hover:bg-black mouse:hover:border-black duration-300 cursor-pointer">Show {tempData.productCount} Items</button>
                    <button onClick={clearFilterOnClick} class="font-serif font-semibold text-center border-black border py-2 relative after:absolute after:w-full after:h-0 after:bottom-0 after:left-0 after:bg-black after:duration-300 mouse:hover:after:h-0.5 touchscreen:after:hidden">Clear Filter</button>
                </div>
            </div>
        </div>
    )
